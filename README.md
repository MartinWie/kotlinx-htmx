...
Use `PageSecurityContext.withNonce` to set a CSP nonce that is automatically
added to every `<script>` tag generated by `addJs` and `addDeferredJs`:

```kotlin
val nonce = generateSecureNonce()
response.header("Content-Security-Policy", "script-src 'nonce-$nonce'")

val html = PageSecurityContext.withNonce(nonce) {
    buildHTMLString {
        // Scripts will automatically include the nonce attribute
...
`PageSecurityContext` stores the nonce in a `ThreadLocal`. The `withNonce` helper
is safe as long as the block does not suspend across dispatcher boundaries.

If you need the nonce inside a suspending block (e.g. in Ktor coroutines),
propagate the `ThreadLocal` with `kotlinx-coroutines`:
...
withContext(PageSecurityContext.nonceThreadLocal.asContextElement(nonce)) {
    // scriptNonce is available on any dispatcher thread
    val html = buildHTMLString { addJs("console.log('safe!')") }
}
```
...
| `PageSecurityContext.withNonce(nonce) { }` | Execute block with CSP nonce active |
| `PageSecurityContext.scriptNonce` | Get/set CSP nonce for scripts |
| `PageSecurityContext.nonceThreadLocal` | ThreadLocal for coroutine propagation |
